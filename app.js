...
async function audit(user, action, cedula, detail = '') {
  await pool.query('INSERT INTO auditoria (usuario, accion, cedula, detalle, created_at) VALUES ($1, $2, $3, $4, $5)', [user, action, cedula || null, detail, now()]);
}
  ...
  try {
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      for (const row of rows.slice(headerIndex + 1)) {
        ...
        const result = await client.query(`INSERT INTO empleados (cedula, nombre, cargo, dependencia, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)
          ...
          WHERE empleados.estado = 'PENDIENTE'`, [cedula, nombre, cargo, dependencia, now(), now()]);
...
app.get('/api/empleado/:cedula', async (req, res) => {
  if (!isPublicTokenValid(req)) return res.status(403).json({ error: 'Enlace no autorizado.' });
  const { rows } = await pool.query('SELECT cedula, nombre, cargo, dependencia, estado, fecha_autorizacion, pdf_path FROM empleados WHERE cedula = $1', [normalizeCedula(req.params.cedula)]);
  const employee = rows[0];
  if (!employee) return res.status(404).json({ error: 'Empleado no encontrado. Verifica la cédula e inténtalo nuevamente.' });
  res.json({ ...employee, date: colombiaDate(), authorization: authorizationText(employee) });
});

app.post('/api/autorizaciones', async (req, res) => {
  if (!isPublicTokenValid(req)) return res.status(403).json({ error: 'Enlace no autorizado.' });
  const cedula = normalizeCedula(req.body.cedula);
  const ciudad = clean(req.body.ciudad).slice(0, 80);
  const { rows } = await pool.query('SELECT * FROM empleados WHERE cedula = $1', [cedula]);
  const employee = rows[0];
  if (!employee) return res.status(404).json({ error: 'Empleado no encontrado. Verifica la cédula e inténtalo nuevamente.' });
  if (employee.estado === 'AUTORIZADO') return res.status(409).json({ error: 'Esta autorización ya fue registrada.', employee });
  if (req.body.autorizacionAceptada !== true) return res.status(400).json({ error: 'Debes marcar la casilla de consentimiento.' });
  const signature = clean(req.body.firma);
  if (!/^data:image\/png;base64,[A-Za-z0-9+/=]+$/.test(signature) || signature.length < 200) return res.status(400).json({ error: 'La firma es obligatoria y debe ser válida.' });
  const acceptedAt = colombiaDate();
  const updated = { ...employee, ciudad, estado: 'AUTORIZADO', fecha_autorizacion: acceptedAt, firma: signature, autorizacion_aceptada: 1, version_autorizacion: config.policyVersion };
  const pdf = await createPdf(updated);
  await pool.query(`UPDATE empleados SET estado='AUTORIZADO', fecha_autorizacion=$1, firma=$2, autorizacion_aceptada=1, version_autorizacion=$3, pdf_path=$4, updated_at=$5 WHERE cedula=$6 AND estado='PENDIENTE'`, [acceptedAt, signature, config.policyVersion, pdf.relative, now(), cedula]);
  const { rows: savedRows } = await pool.query('SELECT cedula, nombre, estado, fecha_autorizacion, pdf_path FROM empleados WHERE cedula = $1', [cedula]);
  res.json({ message: 'Autorización guardada correctamente.', employee: savedRows[0], download: `/api/pdf/${encodeURIComponent(pdf.relative)}?token=${encodeURIComponent(config.qrToken)}` });
});

app.post('/api/admin/login', loginLimiter, async (req, res) => {
  const password = clean(req.body.password);
  const hash = process.env.ADMIN_PASSWORD_HASH;
  if (!hash || !password || !(await bcrypt.compare(password, hash))) return res.status(401).json({ error: 'Credenciales inválidas.' });
  req.session.admin = true;
  await audit('admin', 'LOGIN', null);
  res.json({ ok: true });
});
...
app.get('/api/admin/empleados', requireAdmin, async (req, res) => {
  const params = [];
  let sql = 'SELECT id, cedula, nombre, cargo, dependencia, estado, fecha_autorizacion, pdf_path FROM empleados WHERE 1=1';
  if (req.query.estado && ['PENDIENTE', 'AUTORIZADO'].includes(req.query.estado)) { sql += ' AND estado = $' + (params.length + 1); params.push(req.query.estado); }
  if (req.query.dependencia) { sql += ' AND dependencia ILIKE $' + (params.length + 1); params.push(`%${clean(req.query.dependencia)}%`); }
  if (req.query.q) { sql += ' AND (cedula ILIKE $' + (params.length + 1) + ' OR nombre ILIKE $' + (params.length + 1) + ' OR cargo ILIKE $' + (params.length + 1) + ')'; const q = `%${clean(req.query.q)}%`; params.push(q); }
  sql += ' ORDER BY nombre ASC';
  const { rows: employees } = await pool.query(sql, params);
  const { rows: totalsRows } = await pool.query("SELECT COUNT(*)::int total, COALESCE(SUM(CASE WHEN estado='AUTORIZADO' THEN 1 ELSE 0 END), 0)::int autorizado FROM empleados");
  const totals = totalsRows[0];
  res.json({ employees, totals: { total: totals.total, autorizado: totals.autorizado || 0, pendientes: totals.total - (totals.autorizado || 0), avance: totals.total ? Math.round((totals.autorizado || 0) * 100 / totals.total) : 0 } });
});

app.post('/api/admin/empleados', requireAdmin, async (req, res) => {
  const employee = validateEmployeeInput(req.body);
  if (!employee) return res.status(400).json({ error: 'Completa una cédula válida y el nombre.' });
  try {
    await pool.query('INSERT INTO empleados (cedula, nombre, cargo, dependencia, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6)', [employee.cedula, employee.nombre, employee.cargo, employee.dependencia, now(), now()]);
    await audit('admin', 'CREAR_EMPLEADO', employee.cedula);
    res.status(201).json({ ok: true });
  } catch (_) { res.status(409).json({ error: 'La cédula ya existe.' }); }
});

app.patch('/api/admin/empleados/:cedula', requireAdmin, async (req, res) => {
  const employee = validateEmployeeInput({ ...req.body, cedula: req.params.cedula });
  if (!employee) return res.status(400).json({ error: 'Datos inválidos.' });
  const result = await pool.query('UPDATE empleados SET nombre=$1, cargo=$2, dependencia=$3, updated_at=$4 WHERE cedula=$5', [employee.nombre, employee.cargo, employee.dependencia, now(), employee.cedula]);
  if (!result.rowCount) return res.status(404).json({ error: 'Empleado no encontrado.' });
  await audit('admin', 'EDITAR_EMPLEADO', employee.cedula);
  res.json({ ok: true });
});
...
app.post('/api/admin/importar-empleados', requireAdmin, upload.single('archivo'), async (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Adjunta un archivo CSV, XLSX o XLS.' });
  try {
    const result = await importWorkbook(req.file.path);
    fs.rmSync(req.file.path, { force: true });
    await audit('admin', 'IMPORTAR_EMPLEADOS', null, JSON.stringify(result));
    res.json(result);
  } catch (_) { fs.rmSync(req.file.path, { force: true }); res.status(400).json({ error: 'No fue posible leer el archivo. Revisa sus columnas.' }); }
});

app.get('/api/pdf/:relative(*)', (req, res) => {
  if (!req.session.admin && !isPublicTokenValid(req)) return res.status(401).json({ error: 'Acceso no autorizado.' });
  const relative = req.params.relative;
  const full = path.resolve(DATA_DIR, relative);
  if (!full.startsWith(path.resolve(UPLOAD_DIR)) || !fs.existsSync(full)) return res.status(404).end();
  res.download(full);
});

app.get('/api/admin/qr', requireAdmin, async (req, res) => {
  const url = `${config.publicUrl}/?token=${encodeURIComponent(config.qrToken)}`;
  const dataUrl = await QRCode.toDataURL(url, { width: 800, margin: 2, errorCorrectionLevel: 'H' });
  res.json({ url, dataUrl });
});

app.get('/api/admin/descargar-autorizaciones', requireAdmin, async (req, res) => {
  const { rows: employees } = await pool.query('SELECT cedula,nombre,cargo,dependencia,estado,fecha_autorizacion,pdf_path FROM empleados ORDER BY nombre');
  res.attachment(`respaldo_autorizaciones_${new Date().toISOString().slice(0,10)}.zip`);
  const archive = archiver('zip', { zlib: { level: 9 } });
  archive.on('error', (error) => res.status(500).end(error.message));
  archive.pipe(res);
  const csv = ['cedula,nombre,cargo,dependencia,estado,fecha_autorizacion,archivo_pdf', ...employees.map((e) => [e.cedula,e.nombre,e.cargo,e.dependencia,e.estado,e.fecha_autorizacion || '',e.pdf_path ? path.basename(e.pdf_path) : ''].map((v) => `"${String(v).replace(/"/g, '""')}"`).join(','))].join('\n');
  archive.append(csv, { name: 'control_autorizaciones.csv' });
  for (const employee of employees) if (employee.pdf_path && fs.existsSync(path.join(DATA_DIR, employee.pdf_path))) archive.file(path.join(DATA_DIR, employee.pdf_path), { name: `pdf/${path.basename(employee.pdf_path)}` });
  archive.finalize();
});
...
(async () => {
  await initDatabase();
  if (SOURCE_XLSX) {
    const result = await importWorkbook(path.join(ROOT, SOURCE_XLSX));
